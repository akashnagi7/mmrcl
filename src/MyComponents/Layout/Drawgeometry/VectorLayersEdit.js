/* The below code is a React component that creates and manages vector layers on an OpenLayers map. It
uses various OpenLayers components and interactions to draw, modify, and select vector features on
the map. The component receives props for configuring the style and behavior of the vector layers.
It also includes functionality to convert the vector features to GeoJSON format and send them to a
server for further processing or storage. The component is connected to the Redux store to manage
the state of the vector layer properties. */
...
const VectorLayersEdit = (props) => {
    ...
    const addInteraction = () => {
        ...
        // Update the selected features list when selection changes
    ...
    useEffect(() => {
        if (!map) return;
        // // Load initial GeoJSON data and set it as originalGeoJSON state
        // const initialGeoJSONData = /* Load your GeoJSON data here */
        // setOriginalGeoJSON(initialGeoJSONData);
        ...
        // =======================================================
        // // Create a Modify interaction for editing features
        // modify = new Modify({
        //     source: vectorSource.current,
        // });

        // // Listen for modifyend event to capture edited features
        ...
        //     editedFeatures.current = features;
        //     console.log('Edited Features:', editedFeatures.current);

        //     const AgeoJSONFormat = new GeoJSON();
        //     const AgeoJSONData = AgeoJSONFormat.writeFeaturesObject(features);
        //     console.log('GeoJSON Edited Features data:', AgeoJSONData);
    ...
    const updateVectorLayerToGeoJSON = () => {
        ...
        // =======================================================
        // Convert only edited features to GeoJSON
        // const editedGeoJSON = geoJSONFormat.writeFeaturesObject(features, {
        //     featureProjection: 'EPSG:3857', // Change to your desired projection
        // });

        // console.log('GeoJSON data 2:', editedGeoJSON);
        // Compare editedGeoJSON with originalGeoJSON and update only if changes occurred
        // if (JSON.stringify(editedGeoJSON) !== JSON.stringify(originalGeoJSON)) {
        //     // Call an API or function to update the server-side data with editedGeoJSON
        ...
        axios
            .post(
                `/services/SpatialNonSpatialEditing/${props.idOfLayer}/`,
                {
                    action: "Update",
                    editType: "Spatial",
...
export default connect(mapStateToProps, mapDispatchToProps)(VectorLayersEdit);