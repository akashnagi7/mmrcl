/**
 * The below code is a React component that allows users to draw vector shapes on a map and customize
 * the stroke color, fill color, and stroke width of the shapes.
 */
...
const Drawvector = (props) => {
  ...
  const handlevectordrawtoggleChange = (event) => {
    const chklength = props.myname.filter(person => person.checked == true).length;
    if (props.myvectormeasuretoggle) {
      setLayerShow(true);
      setLayerMessage("Already active in measure tools");

    } else if (props.myvectoredittoggle) {
      setLayerShow(true);
      setLayerMessage("Already active in edit tools");

    } else {

      setVectordrawtoggle(!vectordrawtoggle);
      props.changevectordrawtoggle(!vectordrawtoggle);
        ...
        <div className="row">
          ...
          >
            <div class="form-check">
              <input
                ...
                id="vectordrawtoggle"
                checked={vectordrawtoggle}
                onChange={handlevectordrawtoggleChange}
              />
              <label class="form-check-label" for="vectordrawtoggle">
                Draw Mode
              </label>
            </div>
          </div>
          ...
          {vectordrawtoggle ? (
            ...
            >
              ...
              <option value="Circle">Circle</option>
                ...
                >
                  ...
                  <Tooltip title="Draw Point">
                    <ToggleButton id="tbg-radio-1" onClick={handlePoint}>
                      <i class="fa fa-circle"></i>
                    </ToggleButton>
                  </Tooltip>

                  <Tooltip title="Draw Linestring">
                    <ToggleButton id="tbg-radio-2" onClick={handleLine}>
                      <i class="fa fa-ellipsis-v" aria-hidden="true"></i>
                    </ToggleButton>
                  </Tooltip>
                  <Tooltip title="Draw Polygon">
                    <ToggleButton id="tbg-radio-3" onClick={handlePolygon}>
                      <i class="fa fa-square-o" aria-hidden="true"></i>
                    </ToggleButton>
                  </Tooltip>
                  <Tooltip title="Draw Circle">
                    <ToggleButton id="tbg-radio-4" onClick={handleCircle}>
              ...
              {/* <div className="col-md-12">
            <button type="button" class="btn btn-block btn-success" onClick={updateVectorLayerToGeoJSON}>Save</button>
          </div> */}
            </>
          ) : null}
        </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    changevectormeasuretoggle: (vectormeasuretoggle) => {
      dispatch({ type: "CHANGE_VECTORMEASURETOGGLE", payload: vectormeasuretoggle });
    },
...
export default connect(mapStateToProps, mapDispatchToProps)(Drawvector);
